...
/**
 * RedissonCache 单元测试（基于 Redisson RMapCache 的分布式缓存）
 */
public class RedissonCacheTest {
    ...
    public void setUp() {
        Prop prop = new Prop();
        prop.getProperties().setProperty("cache.type", "redisson");
    ...
    public void testGetType() {
        assertEquals("redisson", cache.getType());
    }
    ...
    public void testGetTtl_ExistingKey() {
        ...
        // 应该 > 0（剩余秒数），-1 表示永久
        assertTrue("TTL should be > 0 or -1, but was: " + ttl, ttl > 0 || ttl == -1);
    }
    ...
    public void testGetWithSupplier_SupplierReturnsNull() {
        ...
        assertNull(result);
        // nullValue=true 时应存入哨兵
        assertTrue(cache.exists("user", "supplier-null-key"));
    }

    @Test(expected = IllegalArgumentException.class)
    public void testBuildStorageKey_BlankCacheName_ThrowsException() {
        cache.buildStorageKey("", "key");
    }

    @Test(expected = IllegalArgumentException.class)
    public void testBuildStorageKey_BlankKey_ThrowsException() {
        cache.buildStorageKey("user", "");
    }
    ...
    public void testNullValue_GetWithSupplier_DoesNotPenetrate() {
        // 先存入 null 哨兵
        cache.put("user", "null-key", null);
        // get with supplier 应该识别哨兵，直接返回 null，不穿透
