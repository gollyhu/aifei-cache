...
/**
 * LettuceCache 单元测试（基于 Lettuce RedisClient 的分布式缓存）
 */
public class LettuceCacheTest {
    ...
    public void setUp() {
        Prop prop = new Prop();
        prop.getProperties().setProperty("cache.type", "lettuce");
    ...
    public void testPutAndGet_Integer() {
        ...
        // Lettuce 使用 toString() 存储，返回 String
        assertEquals("123", cache.get("user", "key1"));
    }

    @Test
    public void testPutAndGet_Long() {
        cache.put("user", "key1", 999L);
        assertEquals("999", cache.get("user", "key1"));
    }

    @Test
    public void testPutAndGet_Double() {
        cache.put("user", "key1", 3.14);
        assertEquals("3.14", cache.get("user", "key1"));
    }
    ...
    public void testGetType() {
        assertEquals("lettuce", cache.getType());
    }
    ...
    @Test
    public void testEvict_MultipleKeys() {
        cache.put("user", "k1", "v1");
        cache.put("user", "k2", "v2");
        cache.put("user", "k3", "v3");

        cache.evict("user", "k1", "k2");

        assertNull(cache.get("user", "k1"));
        assertNull(cache.get("user", "k2"));
        assertEquals("v3", cache.get("user", "k3"));
    }
